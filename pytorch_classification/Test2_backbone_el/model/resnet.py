...
class ResNet(nn.Module):  # 整个网络的框架部分

    def __init__(self,
                 block,  # 残差结构，根据不同的层结构传入不同的block
                 blocks_num,  # 例ResNet50 [3, 4, 6, 3]
                 num_classes=1000,
                 include_top=True,  # 是否训练最后一层
                 groups=1,  # 输入和输出通道不分组
                 width_per_group=64):
    ...
    def _make_layer(self, block, channel, block_num, stride=1):
        downsample = None
        if stride != 1 or self.in_channel != channel * block.expansion:  # 添加一个1x1和一个BN
